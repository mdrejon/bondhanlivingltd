    ...
    <AdminLayout>
        <div class="space-y-6">
            ...
            <div class="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <h1 class="text-lg font-semibold text-gray-800">Income Report</h1>
                    <p class="text-xs text-gray-400 mt-0.5">Revenue, advance & due amounts grouped by date</p>
                </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-white rounded-lg shadow-sm p-4 border-l-4 border-blue-500">
                    <p class="text-xs text-gray-400 font-medium">Advance Collected</p>
                    <p class="text-2xl font-bold text-gray-800 mt-1">{{ fmt(summary.total_advance) }}</p>
                </div>
                <div class="bg-white rounded-lg shadow-sm p-4 border-l-4 border-orange-500">
                    <p class="text-xs text-gray-400 font-medium">Due Amount</p>
                    <p class="text-2xl font-bold text-gray-800 mt-1">{{ fmt(summary.total_due) }}</p>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right text-xs font-semibold text-gray-600">Revenue (BDT)</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr v-for="row in rows" :key="row.date_label" class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-right text-blue-700">{{ fmt(row.advance_payment) }}</td>
                        ...
                        <tr v-if="rows.length > 0" class="bg-gray-50 font-semibold border-t-2 border-gray-300">
                            ...
                            <td class="px-4 py-3 text-right text-blue-700">{{ fmt(summary.total_advance) }}</td>
                            <td class="px-4 py-3 text-right text-orange-600">{{ fmt(summary.total_due) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
const props = defineProps({
    ...
    report_type: { type: String, default: 'income' },
})
...
function applyFilters() {
    router.get(route('admin.reports.income'), { ...localFilters.value }, {
        preserveState: true,
        replace: true,
    })
}

// Auto-apply when year or month changes (the selects call applyFilters via @change)
